...
A separate mobile web app is available in [`phone/`](phone/). It runs in your phone browser and talks to Gemini through a small Cloudflare Worker; it does not require the desktop app or a PC. See the [phone-only deployment guide](phone/README.md). The phone version provides AI chat and browser-based voice features, not desktop control or background wake-word listening.
